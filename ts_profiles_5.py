        ...
        for field in ['ne','te']:
            ...
            for iatt in range(nattempts):
                ...
                try:
                    ...
                    delt = timer() - t1
                    print('  Run {:02d} of {:02d}  chi2_reduced {:8.1f} with {:3d} iter ({:.3f} s)'.
                          format(iatt+1, nattempts, info.chi2_reduced, info.niter, delt))
                except:
            ...
            # end fitting loop
            delt = timer() - t_fitloop
            print('Elapsed time: {:.3f} s'.format(delt))
            ...
            if not noplot:
                ...
                plt.tight_layout()
                if savefig:
                    plt.savefig('fit_params_{}_{:02d}.pdf'.format(field,ipro+1),
                                transparent=True)
                ...
                plt.tight_layout()
                if savefig:
                    plt.savefig('fit_{}_{:02d}.pdf'.format(field,ipro+1),
                                transparent=True)
            # end plot section
            
        # end loop over ne/te fields
